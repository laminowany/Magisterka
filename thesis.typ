  ...
  **/
  ...
  show: wut-thesis.with(
    ...
    title: (
      ...
      pl: "Ewolucja architektur grafowych sieci neuronowych z wykorzystaniem kartezjańskiego programowania genetycznego w zastosowaniu do CVRP",
    ),
    abstract: (
      en: [
      ...
      pl: [
        Niniejsza praca bada, czy kartezjańskie programowanie genetyczne (CGP, ang. Cartesian Genetic Programming) może być skutecznie wykorzystane jako metoda automatycznego wyszukiwania architektur sieci neuronowych (NAS, ang. Neural Architecture Search) do ewolucji architektur enkodera dla problemu marszrutyzacji pojazdów z ograniczoną ładownością (CVRP, ang. Capacitated Vehicle Routing Problem). Praca bazuje na modelu zaproponowanym w literaturze przez Koola i in., który wykorzystuje enkoder oparty na architekturze transformer do rozwiązywania problemów marszrutyzacji. Wyszukiwanie architektury jest ograniczone wyłącznie do enkodera, podczas gdy dekoder oraz ogólny proces uczenia pozostają niezmienione. 
        ...
        Uzyskane wyniki pokazują, że NAS oparty na CGP może skuteczniej kierować procesem wyszukiwania architektur niż wyszukiwanie losowe w rozważanej konfiguracji oraz pozwala odkrywać skuteczne modyfikacje istniejącego enkodera opartego na architekturze transformer. Jednocześnie niewielkie różnice po pełnym treningu oraz ograniczona liczba pełnych ewaluacji wskazują na potrzebę dalszych badań przed sformułowaniem bardziej ogólnych wniosków.
      ]
    ),
    keywords: (

...
While neural network architectures are often designed manually, there also exist methods for automatically discovering new architectures. This task is addressed by @nas:long (@nas), a family of techniques aimed at automating the design of neural network architectures. @nas methods have demonstrated promising results in various domains, reducing the need for manual architecture engineering and, in some cases, discovering architectures that outperform manually designed alternatives @NAS_survey. However, applying @nas to @gnn:pl for combinatorial optimization problems remains computationally expensive, which motivates the use of constrained search spaces and efficient evolutionary strategies.

...
The scope of this work is limited to the CVRP and does not consider other variants of the Vehicle Routing Problem. Furthermore, the study focuses on evaluating the effectiveness of CGP-based architecture search rather than achieving state-of-the-art routing performance. In the first experiment, evolved architectures are compared against architectures obtained via random search under the same computational budget. In the second experiment, the CGP-based NAS approach is used to improve the original encoder architecture of the baseline model.

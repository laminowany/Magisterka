...
=== Hopfield Network
The first application of neural networks to routing problems dates back to 1985, when Hopfield and Tank formulated the @tsp as an energy minimization problem using a recurrent neural network (Hopfield network) @Hopfield. They describe the network through its energy function, where the optimal path is represented by the state with the lowest energy. All constraints such as visiting each node only once are modeled through penalties in the energy function. Such constraints, which can be broken but are discouraged through the cost function, are referred to as soft constraints.
...
Unlike the feed-forward encoder introduced by Nazari et al. @Nazari, the 
...
The encoder also computes an embedding for the entire graph $overline(h)^"(N)"$, by taking an average of all final node embeddings:
$ overline(h)^"(N)"  = 1 / n sum_"i=1"^n h_i^"(N)" $ 

The output of the encoder consists of final node embeddings $h_i^"(N)"$ and the graph embedding $overline(h)^"(N)"$.
...
The decoder operates on a context embedding that combines information produced by the encoder with the partial solution constructed so far. For @cvrp, the decoder context combines the graph embedding, the embedding of the current vehicle location, and the remaining vehicle capacity. This context is used to compute the query vector for the attention mechanism, while the encoded node representations serve as keys and values.

Another improvement over @Bello was the replacement of the learned critic network with a rollout baseline for variance reduction in the REINFORCE policy gradient estimator.
Instead of using a separate neural network to estimate the baseline, Kool et al. use a baseline generated by greedily decoding solutions with a frozen copy of the current best policy. Whenever a new policy achieves statistically significant improvement, it replaces the current baseline.
...
Traditionally, @cgp is evolved using a mutation-only strategy. Miller reports that standard one-point crossover was found to disrupt subgraphs encoded in the chromosome and negatively affect the performance of @cgp @MillerCGP2.

...
Instead of using a separate neural network to estimate the baseline, Kool at al. use a baseline generated by greedily decoding solutions with a frozen copy of the current best policy. Whenever a new policy achieves statistically significant improvement, it replaces the current baseline.

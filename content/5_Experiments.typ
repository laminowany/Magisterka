...
In random search, each candidate architecture is generated independently by randomly sampling a new genome from the search space. In CGP-NAS, new candidates are instead generated by mutating the current parent according to the evolutionary procedure described in @methodology. Both methods explore the same search space, but in a different way.

...
The model and training parameters follow the original configuration used by Kool et al. @Kool and were kept unchanged unless stated otherwise. This ensures that the experiments focus on changes to the encoder architecture rather than on additional hyperparameter tuning.
...
1. *Training dataset* - generated randomly on the fly during training of neural network. Its size is determined by the `epoch_size` parameter.
2. *Validation dataset* - a fixed dataset generated offline and used at the end of each training epoch for logging and plotting. In all experiments, the validation dataset contains 10,000 instances. This dataset was generated using seed 2323.
3. *Test dataset* - a fixed dataset generated offline and used to calculate the final evaluation score. It contains 10,000 instances. This dataset was generated using seed 3232.
...
Therefore, in this thesis there are two types of evaluations:
...
The goal of this experiment is to compare the implemented CGP-based @nas method with random search under the same computational budget. Both methods operate on the same architecture search space and use the same proxy evaluation procedure, allowing the effect of the search strategy itself to be evaluated.
...
The size of $15 times 5$ was selected arbitrarily, with the aim of providing a good compromise between these two cases.

...
The results of Experiment I show that CGP-based architecture search performs better than random search under the same search space and computational budget. Across 10 independent runs, CGP-based @nas achieved better proxy scores than random search, both on average and for the best-performing run. The analysis of the search progress also showed that @cgp continued to improve the architectures throughout the search, while random search showed relatively little improvement after the initial part of the search. These results provide a positive answer to RQ1 under the considered experimental setup.
...
The evolved architectures also differ considerably from each other. EVO-3 achieves better results than the single-layer transformer across all evaluated problem sizes while using almost the same number of parameters, EVO-4 uses considerably fewer parameters, while EVO-6 is larger and more complex. This shows that the evolutionary search is able to discover different modifications of the initial architecture rather than simply increasing its size.
...
Another interesting direction would be to investigate architectures composed of multiple evolved encoder layers. In this thesis, the evolutionary search is limited to a single encoder layer. A discovered architecture could be used as a building block and repeated multiple times, for example to construct a three-layer encoder similar to the original model of Kool et al. @Kool. Alternatively, each layer could use a different evolved architecture. This would allow investigating whether the improvements observed for individual evolved layers are preserved or further increased when they are combined into deeper encoder architectures.
